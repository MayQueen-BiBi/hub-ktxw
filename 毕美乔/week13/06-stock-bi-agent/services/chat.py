...
def ensure_chat_session(user_name, content, session_id, task):
    with SessionLocal() as session:
        record = (
            session.query(ChatSessionTable)
...
async def chat(user_name: str, session_id: Optional[str], task: Optional[str], content: str, tools: List[str] = []):
    ...
    # agent 初始化
    external_client = openai_client

    # mcp tools 选择
    if not tools or len(tools) == 0:
        tool_mcp_tools_filter: Optional[ToolFilterStatic] = None
    else: tool_mcp_tools_filter: (
        ToolFilterStatic) = ToolFilterStatic(allowed_tool_names=tools)
    ...
    stock_agent = Agent(
        ...
        tool_use_behavior="run_llm_again",  # 由 agent 自己决定是否 run_llm_again
        model_settings=ModelSettings(parallel_tool_calls=False)
    )
    ...
    triage_agent = Agent(
        name="Triage Agent",
        instructions=f"""
        Handoff to the appropriate agent based on the task of the request.
        If the request requires tools, ONLY handoff to Stock Assistant
        when tools are allowed: {tools} 
        """,
    ...
    assistant_message = ""
    try:
        async with mcp_server:  # 🔥 关键：初始化 & 回收 MCP Server
