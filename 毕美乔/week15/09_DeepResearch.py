...
MODEL_NAME = "qwen-max"
API_KEY = os.getenv("OPENAI_API_KEY", "sk-a74ccef85bab443db01407504119889a")
...
JINA_API_KEY = "jina_ce2970f90ee544998e776289cf714011qXIUJ_e8LWnuQOICbSSo6-bGSS8P"
...
def crawl_jina(url: str) -> str:
    ...
    try:
        ...
        # 使用 r.jina.ai 作为代理
        response = requests.get("https://r.jina.ai/" + url, headers=headers, timeout=60)
...
"""
    ...
    # 改进点：前面的章节其实会影响后面的章节
    # 每个章节依次执行
    # 1. 创建信号量，限制最大同时抓取的数量为 3
    sem = asyncio.Semaphore(2)

    async def process_section(i: int, section: Dict[str, Any]) -> str:
        # 使用 async with 控制并发
        async with sem:
            ...
            # 每个章节有单独的关键词，检索相关的内容
            current_query = f"{section_title} 搜索关键词: {search_keywords}"
            ...
            while attempt <= max_retries:
                print(f"\n--- 章节 {i + 1} (第 {attempt + 1} 次尝试): {section_title} ---")
                # 1. 检索
                section_search_results_str = await async_search_jina(current_query)
                ...
                for url in urls_to_crawl:
                    try:
                        # 建议修改 async_crawl_jina 函数内部的 timeout 为 60
                        content = await async_crawl_jina(url)
                        crawled_content.append(f"--- URL: {url} ---\n{content[:3000]}...")
                    except Exception as e:
                        print(f"Warning: 抓取 {url} 超时或失败，已跳过。")
                        continue

                raw_materials = "\n\n".join(crawled_content)

                # 3. 内容起草 (调用 Drafting Agent)
                draft_prompt = f"""
                **章节主题:** {section_title}
                ...
                **原始网页内容 (请基于此内容撰写):**
                ...
                try:
                    ...
                    print(f"-> 章节起草失败: {e}")
                    draft_content = f"章节起草失败: {e}"  # 给一个默认值

                # 4. 引入 ReflectionAgent 进行审核 (补充你之前漏掉的逻辑)
                reflect_prompt = f"用户提问: {section_title}\n生成回答: {draft_content}\n请判断是否满足要求。"
                reflection = await Runner.run(ReflectionAgent, reflect_prompt)
                ref_output = reflection.final_output.strip()
                if "满足" in ref_output and "不满足" not in ref_output:
                    print(f"<- [审核通过] 章节 {i + 1}")
                    break
                else:
                    attempt += 1
                    if attempt <= max_retries:
                        print(f"<- [审核未通过] 正在根据建议重试。建议: {ref_output[:50]}...")
                        # 将审核出的“子问题”作为下一次搜索的新关键词
                        current_query = f"{section_title} 缺失要点补查: {ref_output}"
                    else:
                        print(f"<- [达到最大重试] 章节 {i + 1} 将采用当前版本。")

        return f"## {section_title}\n\n{draft_content}"

    tasks = [process_section(i, s) for i, s in enumerate(sections)]
